...
methods {
    function restricted()    external returns address envfree;
...
rule onlyOwnerOrPendingOwnerCanChangeOwnership(env e) {
    ...
    assert oldCurrent != newCurrent => (
        ...
        (e.msg.sender == oldCurrent && newCurrent != 0 && f.selector == sig:transferOwnership(address)) ||
        (e.msg.sender == oldCurrent && newCurrent == 0 && f.selector == sig:renounceOwnership())
    );
}
