...
rule onlyGrantCanGrant(env e, method f, bytes32 role, address account) {
    ...
    assert (
        ...
        f.selector == sig:grantRole(bytes32, address)
    );
    ...
    assert (
        ...
        !hasRoleAfter
           ...
           f.selector == sig:revokeRole(bytes32, address) ||
           f.selector == sig:renounceRole(bytes32, address)
